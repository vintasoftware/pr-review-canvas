...
| `generation.models`             | `{}`                                                                        | The model each agent generates canvases with, keyed by the agent id `publish --agent` records, such as `claude: opus`. An agent with no entry keeps the session's model; see [the skill's model rules](../skills/pr-review-canvas/SKILL.md#model-choice) |
