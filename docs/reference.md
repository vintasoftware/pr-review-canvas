...
| `canvas.ignoreMergeCommits`     | `true`                                              | Keep the canvas current when the head only merged the base branch in, as git would have; see [outdated canvases](#outdated-canvases). Set to `false` to mark it outdated on every commit                                   |
