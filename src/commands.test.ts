...
describe('prepare, publish, validate through the CLI layer', () => {
  ...
  it('maps every failure to an envelope line and an exit code', () => {
    const cases: Array<[unknown, number, string, string?]> = [
      ...
      [new GitLabApiError('x', '401 Unauthorized', 1), EXIT.gh, 'GLAB_UNAUTHENTICATED'],
      [new GitLabApiError('x', 'glab: command not found', 127, true), EXIT.gh, 'GLAB_MISSING'],
      [new GitLabApiError('x', 'HTTP 500', 1), EXIT.error, 'GITLAB_API_ERROR'],
