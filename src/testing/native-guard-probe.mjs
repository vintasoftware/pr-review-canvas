...
if (agent === 'codex') {
  const policy = execFileSync(
    process.execPath,
    [fileURLToPath(new URL('../acpx/codex-chat.mjs', import.meta.url)), '--policy'],
    { env, cwd: fixture, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }
  )
...
const server = createServer(async (request, response) => {
  ...
  if (
    ...
    (!request.url.includes('messages') && !request.url.includes('responses'))
  ) {
  ...
  if (agent === 'claude') {
    if (!body.tools?.some(tool => tool.name === 'Bash')) {
      response.end()
      return
    }
    toolOffered = true
    ...
    event('content_block_delta', {
      ...
      delta: call
        ? {
            ...
            partial_json: JSON.stringify({ command, description: 'Disposable guard probe' }),
          }
        : { type: 'text_delta', text: 'Guard probe complete.' },
    })
    ...
    const tool = body.tools?.find(candidate =>
      ['exec_command', 'shell_command', 'shell'].includes(candidate.name)
    )
    if (!tool) {
      response.end()
      return
    ...
    const item = call
      ...
      : {
          ...
          role: 'assistant',
          status: 'completed',
          content: [{ type: 'output_text', text: 'Guard probe complete.', annotations: [] }],
        }
...
try {
  ...
  const visibleDenial = stdout.split('\n').some(line => {
    try {
      ...
      return (
        (update?.status === 'failed' || update?.sessionUpdate === 'agent_message_chunk') &&
        JSON.stringify(update.content).includes('Blocked by dcg')
      )
    } catch {
      return false
    }
  ...
  if (!toolOffered || !toolSent || !denialSeen || !visibleDenial || existsSync(marker)) {
    throw new Error(
      `${agent}: toolOffered=${toolOffered}, toolSent=${toolSent}, denialSeen=${denialSeen}, visibleDenial=${visibleDenial}, executed=${existsSync(marker)}\n${stdout.slice(-6000)}\n${stderr.slice(-2000)}`
    )
  }
  process.stdout.write(
    `${agent}: dcg denied native AWS tool before execution and explained the denial to the model\n`
  )
} finally {
  child?.kill()
