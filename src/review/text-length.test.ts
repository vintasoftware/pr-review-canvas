...
describe('visibleText', () => {
  it('drops link targets, backticks, and fence lines, and keeps everything a reader sees', () => {
    expect(visibleText('See [the store](#chunk:packages/x/store.ts#2) now.')).toBe('See the store now.')
    expect(visibleText('Bare #chunk:packages/x.ts#1 stays')).toBe('Bare #chunk:packages/x.ts#1 stays')
  ...
  it('counts the visible characters', () => {
    expect(visibleLength('[see](#chunk:packages/provider-portal/src/components/shell/surface.tsx#2)')).toBe(3)
