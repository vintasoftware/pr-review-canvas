...
import { sameChangeSet } from './canvas-lookup.js'
...
async function currentHead(
  ...
  const [prepared, head] = await Promise.all([
    ctx.derived.read(context.headSha),
    ctx.derived.ensure(shas.headSha, shas.mergeBaseSha),
  ])
  return { headSha: meta.headSha, moved: prepared === null || !sameChangeSet(prepared, head) }
}
