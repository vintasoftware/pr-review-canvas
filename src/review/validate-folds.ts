...
/**
 * Above this many diff rows, a file with no attention point and no annotation must hide something
 * at some level. The reading-level control exists to take routine code off the screen, and a
 * routine file left fully open at every level defeats it.
 */
export const ROUTINE_FILE_ROWS = 20
 ...
 * `aggressive` it must fold at least `AGGRESSIVE_MIN_HIDDEN` of the rows outside its annotations
 * and attention points. Otherwise aggressive shows the same core as moderate.
 */
...
function fileFolds(
  ...
  return {
    ...
    pinned: pinnedRanges(file, layer, output, hunks),
...
/**
 * The rules a fresh generation meets, about what the reading levels must be given to hide: a test
 * file keeps its titles at light, a light fold is one generated block, a routine file over
 * `ROUTINE_FILE_ROWS` hides something somewhere, and an open core file folds half of what the
 * reviewer need not judge by aggressive. A stored canvas may predate them.
 */
function generationErrors(
  { file, hunks, where, pinned, annotations, folds, collapsed }: FileFolds,
  isTest: boolean
): ValidationError[] {
  const errors: ValidationError[] = []
  ...
  const routine = pinned.length === 0 && annotations.length === 0
  if (routine && rows > ROUTINE_FILE_ROWS && folds.length === 0) {
    errors.push(
      error(
        'FOLD_MISSING',
        where,
    ...
    // there is the page's own count. The judged rows are what annotations and points do not cover.
    const judged = Math.max(rows - coveredRows([...pinned, ...annotations]), 0)
      ...
      errors.push(
        error(
          ...
          `${hidden} of ${rows} changed lines hide at aggressive; a core file keeps its defining ` +
            `lines and annotations and folds the rest, at least half of the ${judged} lines outside them`
        )
      )
    }
  }
...
export function validateFolds(
  ...
  return output.layers.flatMap(layer =>
    layer.files.flatMap(file => {
      const folds = fileFolds(file, layer, output, byPath.get(file.path)?.hunks ?? [])
      return options.storedArtifact === true
        ? correctnessErrors(folds)
        : [...correctnessErrors(folds), ...generationErrors(folds, isTestPath(file.path, testPatterns))]
    })
  )
}
