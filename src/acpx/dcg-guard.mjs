...
export function evaluate(command, binary = process.env.PR_REVIEW_DCG_BIN ?? 'dcg') {
  if (typeof command !== 'string' || !command.trim() || command.length > 128_000) {
    return {
      allowed: false,
      reason: 'Chat command guard: missing, empty, or oversized command. Nothing was executed.',
    }
  }
  const result = spawnSync(
    binary,
    ['test', '--robot', '--config', path.join(policyDir, 'dcg-policy.toml'), '--', command],
    {
      cwd: policyDir,
  ...
  try {
    ...
    if (result.status === 1 && verdict.decision === 'deny' && typeof verdict.reason === 'string') {
      return {
        allowed: false,
        reason: `Blocked by dcg (${verdict.rule_id ?? 'chat policy'}): ${verdict.reason}\n${verdict.explanation ?? ''}\nThis command was not executed. Do not bypass the guard.`,
      }
    }
  } catch {
    /* Any unexpected evaluator result blocks the command. */
  }
  return {
    allowed: false,
    reason:
      'Chat command guard could not verify this command (dcg unavailable, timed out, or returned an invalid result). Nothing was executed. Repair dcg and run pr-review doctor before retrying.',
  }
}
...
export function checkGuard() {
  for (const command of [
    ...
    if (verdict.allowed || !verdict.reason.startsWith('Blocked by dcg (')) {
      throw new Error(
        `Required dcg chat policy failed its non-executing probe for: ${command}. ${verdict.reason ?? ''}`
      )
    }
  }
  if (!evaluate('git status --short').allowed)
    throw new Error('Required dcg chat policy rejected its read-only probe.')
}
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  if (process.argv[2] === '--ready') {
    ...
    try {
      checkGuard()
      process.stdout.write('dcg chat policy verified\n')
    } catch (error) {
    ...
    let output
    try {
      output = hookResult(JSON.parse(readFileSync(0, 'utf8')))
    } catch {
      output = deny('Chat command guard could not read the hook input. Nothing was executed.')
    }
