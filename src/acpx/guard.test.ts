...
it('denies dangerous remote commands and evaluator failures with a reason', () => {
  const command = hostCommand('node', [
    agentPath(fileURLToPath(new URL('../testing/dcg-guard-probe.mjs', import.meta.url))),
  ])
  expect(
  ...
  ).toContain('override isolation passed')
}, 45_000)

it.each(['codex', 'claude'])(
  '%s blocks a real native tool call before the harmless fixture executable runs',
  agent => {
    const command = hostCommand('node', [
      agentPath(fileURLToPath(new URL('../testing/native-guard-probe.mjs', import.meta.url))),
      agent,
    ])
    expect(
      execFileSync(command.file, command.args, {
    ...
    ).toContain('explained the denial to the model')
  },
  120_000
)
