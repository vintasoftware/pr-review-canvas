...
try {
  ...
  try {
        ...
        SessionStart: [
          {
            hooks: [
              {
                type: 'command',
                command: `${guardCommand} --ready ${quote(marker)} ${quote(nonce)}`,
                timeout: 15,
              },
            ],
          },
    ...
    execFileSync(
      binary,
      ['--init-only', '--setting-sources', '', '--settings', JSON.stringify(probeSettings)],
      {
        encoding: 'utf8',
        timeout: 30_000,
    ...
    if (readFileSync(marker, 'utf8') !== nonce)
      throw new Error('Claude did not activate its required chat hook.')
  } finally {
  ...
  if (process.argv[2] === '--check') process.stdout.write('Claude dcg launch hooks verified\n')
    ...
    for (let index = 0; index < input.length; index++) {
      if (input[index] === '--settings' || input[index] === '--setting-sources') {
        index++
        continue
      }
