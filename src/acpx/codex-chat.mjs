...
function listHooks(binary, args) {
  return new Promise((resolve, reject) => {
    ...
    lines.on('line', line => {
      try {
        ...
        if (message.id === 1) {
          ...
          child.stdin.write(
            `${JSON.stringify({ id: 2, method: 'hooks/list', params: { cwds: [process.cwd()] } })}\n`
          )
    ...
    child.stdin.write(
      `${JSON.stringify({ id: 1, method: 'initialize', params: { clientInfo: { name: 'pr-review-guard', version: '1' }, capabilities: { experimentalApi: true } } })}\n`
    )
  })
}
  ...
  try {
    ...
    if (reason)
      output.push(
        JSON.stringify({
          method: 'warning',
          params: { threadId: message.params.threadId, message: reason.slice(0, 350) },
        })
      )
  } catch {
    /* Preserve protocol errors for the adapter to report. */
  }
  return output
}
...
try {
  ...
  const entry = first.data?.find(candidate => candidate.cwd === process.cwd())
  const hook = entry?.hooks.find(
    candidate =>
      candidate.source === 'sessionFlags' &&
      candidate.eventName === 'preToolUse' &&
      candidate.command === preToolUse[0].hooks[0].command &&
      candidate.matcher === '^Bash$' &&
      candidate.async === false
  )
  if (!hook || entry.errors?.length || typeof hook.currentHash !== 'string' || typeof hook.key !== 'string') {
    throw new Error(
      'Codex does not expose the required chat hook. Install a Codex version supporting PreToolUse and hooks/list (tested with 0.154.0).'
    )
  }
  if (policy) {
    ...
    if (!hook.enabled || hook.trustStatus !== 'trusted')
      throw new Error(
        'Codex did not activate its exact dcg hook. Stop the server, remove this checkout’s chat runtime, and retry.'
      )
