...
let fixture: string
beforeEach(async () => { fixture = await realpath(await mkdtemp(path.join(os.tmpdir(), 'chat preparation-'))) })
afterEach(async () => { vi.unstubAllEnvs(); await rm(fixture, { recursive: true, force: true }) })

it.runIf(process.platform !== 'win32')('keeps the event loop responsive and shares preparation across concurrent and later calls', async () => {
...
it.runIf(process.platform !== 'win32')('leaves no published runtime after an import failure and allows retry', async () => {
  const stateRoot = path.join(fixture, 'runtime')
  ...
  } finally { await chmod(credential, 0o600) }
  await launch('/bin/true', [], fixture)
  const entries = await readdir(stateRoot)
  expect(entries).toHaveLength(1)
  expect(await readFile(path.join(stateRoot, entries[0]!, 'home/.claude.json'), 'utf8')).toBe('fixture credentials')
}, 90_000)
