...
vi.mock('./sandbox-client.js', () => ({ createSandboxClient: () => preparation.launch, agentPathAsync: async (file: string) => file }))
...
it('cancels during preparation without spawning the agent after preparation finishes', async () => {
  ...
  preparation.launch.mockReturnValue(new Promise(resolve => { finish = resolve }))
  const run = createAgentRunner({ bin: 'fixture' }).run({ cwd: '/fixture', agent: 'claude', session: 'test', prompt: 'hello', timeoutSec: 10 })
