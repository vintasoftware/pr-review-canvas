...
describe('toAppError', () => {
  it('maps every known error to a code, status, and hint', () => {
    ...
    expect(toAppError(new GitHubApiError('user', 'HTTP 401', 1))).toMatchObject({
      code: 'GH_UNAUTHENTICATED',
      status: 401,
      hint: 'run `gh auth login`',
    })
    expect(toAppError(new GitHubApiError('user', 'HTTP 502', 1))).toMatchObject({
      code: 'GITHUB_API_ERROR',
      status: 502,
    })
    expect(toAppError(new GitHubApiError('user', 'gh: command not found', 1, true))).toMatchObject({
      code: 'GH_MISSING',
      status: 500,
