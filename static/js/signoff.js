...
import { forgeLabel, signoffHostTitle } from './host.js'
...
export function signoffDialogHtml(opts) {
  return (
    ...
    `<p class="hint">This posts a review on ${esc(forgeLabel())} as you. Edit the body first if you want to.</p>` +
...
export function showSignoffResult(dialog, review) {
  ...
  if (result !== null) {
    ...
    result.append(externalLink(review.url, `see it on ${forgeLabel()}`))
  }
  return result
}
