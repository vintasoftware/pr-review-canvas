 ...
 * head only merged the base branch in since, as git would have, so the canvas still applies.
...
export function mergesBarHtml(merges) {
  ...
  return (
    ...
    `${esc(`It was generated for ${canvas}; the head ${head} only merged the base branch in since (${n} commit${n === 1 ? '' : 's'}). The diffs are the head's.`)} ` +
