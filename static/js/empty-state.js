 ...
 * head only merged the base branch in since, with no conflicts, so the canvas still applies.
...
export function mergesBarHtml(merges) {
  ...
  return (
    ...
    `${esc(`It was generated for ${canvas}; the head ${head} only merged the base branch in since (${n} commit${n === 1 ? '' : 's'}, no conflicts). The diffs are the head's.`)} ` +
